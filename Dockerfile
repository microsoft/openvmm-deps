...
# config.sub is vendored in cross/sources/ rather than fetched from gitweb
# (https://git.savannah.gnu.org/gitweb/?p=config.git;...;hb=3d5db9ebe860).
# git.savannah.gnu.org is intermittently unreachable / serves error pages
# instead of the file, breaking unrelated CI builds. The file is pinned to a
# specific git commit and never changes; vendoring it eliminates the network
# dependency without any semantic change. The original sha256 is preserved
# in cross/sources/config.sub.sha256 for provenance.
# cg-add: --checksum=sha256:75d5d255a2a273b6e651f82eecfabf6cbcd8eaeae70e86b417384c8f4a58d8d3 https://git.savannah.gnu.org/gitweb/?p=config.git;a=blob_plain;f=config.sub;hb=3d5db9ebe860
COPY --link cross/sources/config.sub /sources/config.sub
